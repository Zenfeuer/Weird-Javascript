 ...
 * classes. Prototypal inheritance is not more than objects 'inherits' from other objects.
 *
 * All objects have an special property called 'proto', that is another object and helps to implement the prototypal
 ...
 *
 * We are forcing that john object 'inherits' from person object. firstname and lastname properties are already defined
 * in john object, so it does not inherits those properties from person (neither the values).
 */
 ...
 * DON'T DO THIS EVER! For demo purposes only!!!
 * 
 * We are forcing that jane object 'inherits' from person object. firstname property is already defined in jane object, 
 * so it does not inherits that property from person (neither the value).
 */
jane.__proto__ = person;

/** 
 * Now jane object can access getFullName() method. This prints out 'Jane Default', because jane object has not defined
 * lastname property, so by prototype chain it uses the defined one from person object.
 */
 ...
 * IMPORTANT NOTE
 *
 * Mutating an object that is being inherited by other objects, those objects are also going to see/access the new added
...
/**
 * NOTE: Prototype of an Object
 *
 * The prototype of an object is Object {} and it contains basic methods and properties that can be used for all objects 
 * such as toString() or hasOwnProperty().
 */
console.log(a.__proto__);

/**
 * NOTE: Prototype of a Function
 *
 * The prototype of a function is function Empty () {} and it contains basic methods and properties that can be used for 
 ...
 * new object and set 'this' to point to that new object when the function constructor is executed. Also, 'new' operator
 * sets the prototype of that empty object to the prototype property of the function that you then call.
 *
 ...
 * only define the properties in the function constructor (see example below).
 *
 **********************************************************************************************************************/

// A function constructor. By convention/standard, function constructor name is defined with capital letter.
function Person(firstname, lastname)
{
    ...
    /**
     * If you return something in a function constructor, the variable will have the returned value and not the new
     * object.
     */
...
/**
 * NOTE: This add getFullName() to the prototype Person, so the objects created with the function constructor Person 
 * are going to have access to this method because prototype chain.
 */
 ...
 * object, and whatever modification done to that object using the 'this' variable will end up as part of that object 
...
/**
 * NOTE: This add getFormalFullName() to the prototype Person, so the objects created with the function constructor
 * Person are going to have access to this method because prototype chain.
 */
